...
describe('src :: api :: controllers :: rental :: reserve :: update', () => {
  describe('GIVEN PUT to update a reserve', () => {
    describe('WHEN every validation is meth', () => {
      ...
      beforeEach(async () => {
        ...
        response = await request(app)
          .put(`${RENTALRESERVEPREFIX.replace('{id}', fleetInfo.id_locadora)}/${_id?.toString()}`)
          .set(TOKEN)
    ...
    describe('WHEN using nonexistant id_carro', () => {
      ...
      beforeEach(async () => {
        ...
        response = await request(app)
          .put(`${RENTALRESERVEPREFIX.replace('{id}', id_locadora?.toString() as string)}/${_id?.toString()}`)
          .set(TOKEN)
          .send({
    ...
    describe('WHEN using nonexistant id_locadora', () => {
      ...
      beforeEach(async () => {
        ...
        response = await request(app)
          .put(`${RENTALRESERVEPREFIX.replace('{id}', '6171508962f47a7a91938d30')}/${_id?.toString()}`)
          .set(TOKEN)
          .send({
    ...
    describe('WHEN a rental id is invalid', () => {
      ...
      beforeEach(async () => {
        ...
        const { _id } = await factory.create<RentalReserve>('RentalReserve');
        response = await request(app)
          .put(`${RENTALRESERVEPREFIX.replace('{id}', '23')}/${_id?.toString()}`)
          .set(TOKEN)
          .send({
            id_carro: id_carro.toString() as string,
            data_fim: moment(data_fim, 'YYYY-MM-DD HH:mm:ss').format('DD/MM/YYYY'),
    ...
    describe('WHEN a reserve id is invalid', () => {
      let response: request.Response;
      beforeEach(async () => {
        const { data_fim, data_inicio, id_carro, id_locadora } = await factory.build<RentalReserve>('RentalReserve');
        response = await request(app)
          .put(`${RENTALRESERVEPREFIX.replace('{id}', id_locadora?.toString() as string)}/123`)
          .set(TOKEN)
          .send({
            id_carro: id_carro.toString() as string,
            data_fim: moment(data_fim, 'YYYY-MM-DD HH:mm:ss').format('DD/MM/YYYY'),
